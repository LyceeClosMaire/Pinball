...
collisions = pygame.sprite.groupcollide(ball, flippers, False, False, collided = pygame.sprite.collide_mask)

for ball in collisions:
    if collisions[ball] == [FlipperL]:
        normAngleL = -30 + FlipperL.angle + 90
        nx = cos(normAngleL)
        ny = sin(normAngleL)
        (vx , vy) = ball.speed
        ball.speed = 2*(nx*vx) + (ny*vy)
